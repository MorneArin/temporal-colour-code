// TemporalColorCode_AllLUTs.ijm
//
// Runs Kota Miura's Temporal-Color Code core logic (embedded below, dialog
// removed) once per LUT listed under Image > Lookup Tables in Fiji -- the
// same list the plugin's own dropdown offers -- WITHOUT the time color scale
// bar, saves each result as a PNG, and closes it. The original stack is left
// untouched and open throughout.
//
...
macro "Temporal Color Code - All LUTs" {
    ...
    lutA = makeLUTsArray();
    print("Found " + lutA.length + " LUTs under Image>Lookup Tables. Processing '" + srcTitle + "'...");
    ...
    for (lutIndex = 0; lutIndex < lutA.length; lutIndex++) {
        ...
        if (resultID != -1) {
            ...
            savePath = outputDir + safeName + ".png";
            saveAs("PNG", savePath);
            close();
